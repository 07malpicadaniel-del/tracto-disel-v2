import { Search, Undo2 } from 'lucide-react';

interface DevolucionesHeaderProps {
  busqueda: string;
  setBusqueda: (valor: string) => void;
  onNuevaDevolucion: () => void;
}

export default function DevolucionesHeader({ busqueda, setBusqueda, onNuevaDevolucion }: DevolucionesHeaderProps) {
  return (
    <div className="flex flex-col gap-6">
      <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-white tracking-tight">Gestión de Devoluciones</h1>
          <p className="text-slate-400 font-medium text-sm mt-1">Control de garantías, piezas defectuosas y retornos de inventario.</p>
        </div>
        <button 
          onClick={onNuevaDevolucion}
          className="bg-blue-600 hover:bg-blue-500 text-white px-5 py-3 rounded-xl font-bold transition-colors flex items-center gap-2 shadow-lg shadow-blue-600/20 whitespace-nowrap"
        >
          <Undo2 size={20} />
          Registrar Devolución
        </button>
      </div>

      <div className="relative group w-full">
        <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-500 transition-colors" size={20} />
        <input
          type="text"
          placeholder="Buscar por folio de ticket o nombre del cliente..."
          value={busqueda}
          onChange={(e) => setBusqueda(e.target.value)}
          className="w-full bg-slate-900 border border-slate-800 text-white rounded-2xl py-4 pl-12 pr-4 outline-none focus:border-blue-500 transition-all font-medium placeholder:text-slate-500 shadow-sm"
        />
      </div>
    </div>
  );
}